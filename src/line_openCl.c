...
OCL_System setup_opencl(Pixel *h_img, int img_w, int img_h, const char *kernel_source)
{
    ...
    clSetKernelArg(ocl.kernel, 1, sizeof(int), &ocl.img_w);
    clSetKernelArg(ocl.kernel, 2, sizeof(int), &ocl.img_h);
...
void draw_line(OCL_System *ocl, int x1, int y1, int x2, int y2, int radius, pix color)
{
    ...
    clSetKernelArg(ocl->kernel, 3, sizeof(int), &x1);
    clSetKernelArg(ocl->kernel, 4, sizeof(int), &y1);
    clSetKernelArg(ocl->kernel, 5, sizeof(int), &x2);
    clSetKernelArg(ocl->kernel, 6, sizeof(int), &y2);
    clSetKernelArg(ocl->kernel, 7, sizeof(int), &radius);
    clSetKernelArg(ocl->kernel, 8, sizeof(int), &color.r);
    clSetKernelArg(ocl->kernel, 9, sizeof(int), &color.g);
    clSetKernelArg(ocl->kernel, 10, sizeof(int), &color.b);
    clSetKernelArg(ocl->kernel, 11, sizeof(int), &color.a);
...
void fetch_and_average_image(OCL_System *ocl, Image img)
{
    ...
    {
        int count = img->matrix[i].n;
        if (count > 1)
        {
            img->matrix[i].r /= count;
            img->matrix[i].g /= count;
            img->matrix[i].b /= count;
            img->matrix[i].a /= count;
        }
